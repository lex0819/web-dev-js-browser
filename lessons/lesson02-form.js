...
submitBtn.addEventListener('click', () => {
    ...
    questionEls.forEach((questionEl, index) => {
        const checkInput = questionEl.querySelector('input:checked');
        questionEl.classList.toggle('not-check', !checkInput);
        if (checkInput) {
            resultsHtml.push(`
            <p data-number="${index + 1}">
                Вопрос ${index + 1}: 
                <span>${checkInput.value}</span>
            </p>
            `);
        }
    });
